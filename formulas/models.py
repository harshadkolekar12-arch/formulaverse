...
class Chapter(models.Model):
    ...
    def get_absolute_url(self):
        # Generates sitemap URL for chapters
...
class Formula(models.Model):
    ...
    variables = models.CharField(max_length=200, blank=True, null=True)
    chapter = models.ForeignKey(Chapter, on_delete=models.SET_NULL, max_length=50, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            original_slug = slugify(self.title) if self.title else "formula"
            slug = original_slug
            counter = 1
            while Formula.objects.filter(slug=slug).exclude(pk=self.pk).exists():
    ...
    def get_absolute_url(self):
        return reverse('single-formula-page', kwargs={'slug': self.slug})
...
class PYQ(models.Model):
    ...
    def get_absolute_url(self):
        # Generates sitemap URL for PYQs
        return f"/pyq/{self.id}/"
